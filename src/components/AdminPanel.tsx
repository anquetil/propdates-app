...
import { MinimalProp, zeroAddress } from '@/utils/types'
            ...
            <div className='flex flex-col w-full lg:w-4/5 xl:w-1/2'>
               <div className='flex items-center space-x-2 place-self-end w-fit mb-3'>
                  <button
                     ...
                     className={`${
                        showRelevant ? 'bg-gray-800' : 'bg-gray-200'
                     } inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent 
                  transition-colors`}
                  >
                     <span
                        className={`pointer-events-none block h-5 w-5 rounded-full bg-white shadow-lg ring-0 transition-transform 
                     ${showRelevant ? 'translate-x-5' : 'translate-x-0'}`}
                     ></span>
                  </button>
                  <div className='text-sm'>Only Show Relevant Proposals</div>
               </div>
               <div className='divide-y border'>
                  {sortedActions.map((a, i) => {
                     if (
                        !showRelevant ||
                        (showRelevant && a.executed && !a.isCompleted)
                     ) {
                        return (
