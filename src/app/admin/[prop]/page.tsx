...
import { Address, useAccount, useEnsName, useNetwork, useSwitchNetwork } from "wagmi";
...
export default function PropPage({ params }: { params: { prop: string } }) {
   ...
   return (
      <main className='flex min-h-screen flex-col w-3/4 border-x-[1px] border-neutral-200 bg-white px-8 pt-8 pb-4'>
         <Link className="hover:underline mb-3" href='/admin'>← back to Admin</Link>
         <div className={`${mono.className} text-4xl font-semibold mb-8`}>
            Manage Prop #{propId}
         </div>
         ...
         <div className="flex flex-col text-md text-gray-700 border-b-[1.5px] border-neutral-300 mb-4">
            ...
            <div className="mb-2">{`${prop.proposer} ${proposerENS ? `(${proposerENS})`: ''}`}</div>
            <div className="font-semibold">Admin:</div>
            <div className="mb-2">{unclaimed ? 
                  `Unclaimed` : 
                  `${prop.admin} ${adminENS ? `(${adminENS})` : ''}`}
            </div>
            <div>{prop.transferPending ? 
                  `Pending: ${prop.pendingAdmin} ${pendingAdminENS ? `(${pendingAdminENS})` : ''}`
                  : ``}
            </div>
         </div>
         {
