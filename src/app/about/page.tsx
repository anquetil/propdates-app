...
import { MinimalProp, PropUpdate } from '@/utils/types'
import { formatDate } from '@/utils/funcs'
import useGetUpdates from '@/hooks/useGetUpdates'

function makeTableArray(updates: PropUpdate[]): {date: Date, count: number}[]{
   ...
   let result: { date: Date, count: number }[] = [{date: thisMonday, count: 0}]
   const sortedProps = [...updates].sort((a, b) => (a.blockTimestamp < b.blockTimestamp ? -1 : 1))
   const maxDate = new Date(Number(sortedProps[sortedProps.length - 1].blockTimestamp) * 1000)
   let i = 1 // ignore the first prop, wilson's

   while(nextMonday < new Date(maxDate.getTime() + diff) && i < sortedProps.length){ // haven't gone through all props
...
export default function AboutPage() {
   let md =
      'Propdates is a contract built by [wilson](https://twitter.com/WilsonCusack) and a front-end built by [martin](https://twitter.com/anquetil). Its goal is to make it easier for proposers to keep the Nouns community updated on their work. The gas fees of all proposals updates are automatically refunded by the contract.'
   md +=
            ...
            updates && (
               <div className='flex flex-col'>
                  <div className='flex flex-row'>
                     <div className='p-2 text-sm border-y border-l w-44 font-semibold'>Week</div>
                     <div className='p-2 text-sm border w-8  font-semibold text-center'>#</div>
                  </div>
                  {
                     table.map((r, i) => (
                        <div key={i} className='flex flex-row'>
                           <div className='p-2 text-sm border-b border-l w-44'>{formatDate(r.date)}</div>
                           <div className='p-2 text-sm border-b border-x w-8 text-center'>{r.count}</div>
                        </div>
                     ))
                  }
